...
> **Рекомендация: облачный бэкенд (`APP_LLM_BACKEND=openrouter`).** Для большинства сценариев он предпочтительнее локального MLX по двум причинам. Во-первых, выбор моделей не ограничен тем, что помещается в unified memory конкретной машины, — доступны модели существенно сильнее любой локально исполнимой на потребительском железе. Во-вторых, и это касается ЛЮБОЙ локальной модели, включая дефолтную 4B, — MLX обслуживает запросы строго последовательно на одном локальном воркере, а Map-стадия суммаризации бьёт статью на несколько десятков чанков: медленно не потому, что модель маленькая, а потому, что чанки обрабатываются один за другим. На OpenRouter та же Map-стадия отправляет чанки параллельно (см. `APP_OPENROUTER_MAX_CONCURRENCY`) — разница на статье из нескольких десятков чанков ощутима: единицы-десятки секунд против нескольких минут независимо от размера модели. Компромисс — стоимость по API и внешняя зависимость вместо полностью локального инференса; для этого и оставлен `mlx` как дефолт.

> **Время ответа.** Суммаризация статьи целиком — Map-Reduce по нескольким десяткам чанков; на MLX вызовы идут последовательно без батчинга (минуты, таймаут клиента — 900 с), на OpenRouter — параллельно (см. выше). Research-ветка укладывается в 1–3 вызова инструмента независимо от бэкенда.
...
Прогон на 14 статьях из офлайн-корпуса. Модель суммаризации — `qwen/qwen3-30b-a3b-instruct-2507`, судья — заведомо более крупная `qwen/qwen3-235b-a22b-2507`.
...
- **Корпус (`article_sample.json`) — не контролируемая выборка.** Число чанков варьируется от 3 до нескольких десятков на статью, а усреднение per-article не учитывает эту неоднородность.
